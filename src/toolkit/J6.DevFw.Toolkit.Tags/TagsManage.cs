namespace Ops.Plugin.Tags
{
    ...
    public class TagsManage : IHttpHandler,System.Web.SessionState.IRequiresSessionState
    {
        ...
        /// 默认的配置路径
        /// </summary>
        internal static string defaultConfigPath = AppDomain.CurrentDomain.BaseDirectory + "config/tags.conf";
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            //获取配置文件的路径
            configPath = request.QueryString["config"];

            //创建标签管理器对象
            tagsManager = new TagsManager(String.IsNullOrEmpty(configPath) ?
                defaultConfigPath :
            ...
            if (method == "GET")
            {
                string action = request.QueryString["tag.action"];
       ...
       public virtual void Begin_Request()
        ...
        private void SaveTags()
        {
            ...
            foreach (string key in request.Form)
            {
                if (reg.IsMatch(key))
                {
                    id = reg.Match(key).Groups[1].Value;

                    tagsManager.Update(new Tag
                    {
                        Name = request["name_" + id],
                        LinkUri = request["link_" + id],
                        Description = request["des_" + id]
                    });
        ...
        private void Dispaly_TagsList()
        {
            ...
            ds.ReadXml(String.IsNullOrEmpty(configPath)?defaultConfigPath:AppDomain.CurrentDomain.BaseDirectory+configPath);
           ...
           // dv.Sort="index desc";
            ...
            foreach (DataRowView dr in ps)
            {
                sb.Append("<tr><td class=\"center\"><input type=\"checkbox\" class=\"ck\" name=\"")
                    .Append(dr["id"]).Append("\"/></td><td class=\"center\">").Append(dr["id"]).Append("</td>")
                    .Append("<td class=\"center\">").Append(dr["name"]).Append("</td><td>")
                    .Append(dr["linkuri"]).Append("</td><td>")
                    .Append(dr["description"]).Append("</td><td class=\"center\"><a href=\"javascript:;\" onclick=\"edit(this)\">修改</a></td></tr>");
            }


            tagsHtml = sb.ToString();

            string html = reg.Replace(Resource.tagsmanager, match =>
            {
                switch (match.Groups[1].Value)
                {
                    ...
                    case "pagetext": return pagerHtml;
