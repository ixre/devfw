...
//    background:#f9f9f9 url(/public/assets/sys_themes/default/title_bg.gif) 0 5px repeat-x;
//    /*border-bottom:solid 2px #FFF;*/}
//.ui-dialog .box .title .txt{padding-left:25px;background:url(/public/assets/sys_themes/default/blocks.gif) left center no-repeat;}
...
function simpleDialog(dialogOptions) {
    ...
    this.usedrag = dialogOptions.drag;                                                 //是否使用拖拽
    ...
    this.canNotClose = dialogOptions.canClose == false;
...
simpleDialog.prototype._initialize = function () {
    ...
    if (dialog.title) {
        ...
        var titHtml = '<span class="left corner" style="position:absolute;left:0;top:0">&nbsp;</span><span class="txt">'
            + dialog.title;
        if (!this.canNotClose) {
            titHtml += '<span class="close" onclick="window[\'dialog_' + this.id +
                '\'].close()" style="position:absolute;right:5px;top:0;text-decoration:none;font-family:Verdana;cursor:pointer" title="关闭窗口"><span>X</span></span>';
        }
        titHtml += ' </span><span class="right corner" style="position:absolute;right:0;top:0">&nbsp;</span>';
        titleElem.innerHTML = titHtml;
...
simpleDialog.prototype.open = function (uri, width, height, scroll) {
    ...
    this._initialize();
    var pl = this.doc.getElementById('boxcontent_' + this.id);
    pl.innerHTML += "<iframe frameborder='0' scrolling='" + (scroll || 'auto') + "' src='" + uri + "' width='" + (width || '100%') + "' style='padding:0' height='" + (height || '100%') + "'></iframe>";
...
simpleDialog.prototype.getWindow = function () {
    return this.win;
};
...
var SimpleDialog = {
    ...
    ALERT_MSG_TPL : '<div class="ui-alert-message"><span class="alert-icon alert-icon-{icon}"></span>{msg}</div>',
    ...
    customAlert: function (p) {
        var title = this.ALERT_ALL_TITLE + '-' + (p.title ||'');
        ...
        d.write(jr.template(this.ALERT_MSG_TPL, { msg: p.message, icon: p.icon }));
        var buttonsHtml = '';
        for (var i = 0; i < p.buttons.length; i++) {
            var e = p.buttons[i];
            if (e && e.tag) { //兼容IE7
                buttonsHtml += jr.template(this.ALERT_BTN_TPL, { tag: e.tag, text: e.text });
            }
        }
        var rd = Math.floor( Math.random() * 10000);
        d.write('<div class="ui-alert-control" id="ui-alert-control-' + rd + '">' + buttonsHtml + '</div>');
        var pl = d.getWindow().document.getElementById('ui-alert-control-' + rd);
        var btns = pl.getElementsByTagName('INPUT'); //查找点击事件的元素
        for (var i = 0; i < btns.length; i++) {
            if (i === 0) {
                btns[i].focus();
            }
            btns[i].onclick = (function(d, p) {
                return function () {
    ...
    alert: function (msg,func,icon,text) {
        return this.customAlert({
            ...
            message:msg,
            buttons: [
                { tag: 'ok', text: text || this.ALERT_BTN_OK_TEXT }
            ],
            click: function () {
    ...
    confirm: function (msg, func, textArr) {
        var okTxt = this.ALERT_BTN_OK_TEXT;
        var cancelTxt = this.ALERT_BTN_CANCEL_TEXT;
