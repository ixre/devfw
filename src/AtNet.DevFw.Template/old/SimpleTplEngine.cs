...
namespace AtNet.DevFw.Template
{
    ...
    public sealed class SimpleTplEngine
    {
        ...
        /// 数据列正则
        /// </summary>
        private static Regex fieldRegex = new Regex("{([a-z0-9_\\]\\[\u4e00-\u9fa5]+)}");
        ...
        {
            //返回结果
            const string methodPattern = "\\@([a-z_0-9\u4e00-\u9fa5]+)\\(([^)]*)\\)\\s*([\\S\\s]+)@each\\s*off";
                //@each(id){ $nav() }
            ...
            html = methodRegex.Replace(html, m =>
            {
                ...
                if (method == null || method.GetCustomAttributes(typeof (TemplateMethodAttribute), true) == null)
                {
                    return m.Value;
                }
                else
